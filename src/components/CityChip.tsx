...
export const CityChip = ({ onCitySelect, currentCity, showRecenter, onRecenter }: CityChipProps) => {
  ...
  return (
    <div className="absolute top-4 lg:top-16 left-1/2 -translate-x-1/2 z-20 flex items-center gap-1.5">
      ...
      {showRecenter && (
        <button
          onClick={onRecenter}
          className="h-[28px] w-[28px] rounded-full bg-black/80 backdrop-blur-md border border-white/[0.06] shadow-lg hover:bg-black/90 transition-all flex items-center justify-center"
      ...
      {isOpen && (
        <>
          ...
          />
          <div className="absolute top-full mt-1.5 left-0 min-w-[150px] rounded-xl bg-black/90 backdrop-blur-md border border-white/[0.06] shadow-xl overflow-hidden">
            {CITIES.map((city) => (
              <button
                key={city.name}
                onClick={() => {
